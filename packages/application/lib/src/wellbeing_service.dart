...
class SupportAction {
  const SupportAction({required this.id, required this.label, required this.kind});
...
class WellbeingService {
  ...
  List<SupportAction> supportActionsFor(EmotionCheckInInput input) {
    if (input.skipped) {
      return const [SupportAction(id: 'later', label: 'Responder despues', kind: 'pause')];
    }
    ...
    if ((input.overloadLevel ?? 0) >= 3 || input.needsQuiet) {
      actions.add(const SupportAction(id: 'quiet', label: 'Pausa sensorial', kind: 'sensory'));
      actions.add(const SupportAction(id: 'headphones', label: 'Usar auriculares', kind: 'sensory'));
    }
    if (input.needsMovement) {
      actions.add(const SupportAction(id: 'movement', label: 'Mover el cuerpo', kind: 'movement'));
    }
    return actions;
  }
  ...
  Future<EmotionCheckIn> recordCheckIn(EmotionCheckInInput input) {
    ...
    return emotionRepository.save(
      EmotionCheckIn(
        metadata: EntityMetadata(id: 'emotion-${now.microsecondsSinceEpoch}', createdAt: now, updatedAt: now, ownerId: input.profileId),
  ...
  Future<SupportRequest> requestSupport({required String profileId, required SupportAction action}) {
    final now = DateTime.now();
    return supportRepository.save(
      SupportRequest(
        metadata: EntityMetadata(id: 'support-${now.microsecondsSinceEpoch}', createdAt: now, updatedAt: now, ownerId: profileId),
        profileId: profileId,
        kind: action.kind,
        note: action.label,
      ),
  ...
  Future<StoryProgress> markStoryRead({required String profileId, required String storyId, bool favorite = false}) {
    final now = DateTime.now();
    return storyProgressRepository.save(
      StoryProgress(
        metadata: EntityMetadata(id: 'story-progress-$profileId-$storyId', createdAt: now, updatedAt: now, ownerId: profileId),
        storyId: storyId,
        profileId: profileId,
        isFavorite: favorite,
      ),
